...
- 测试用例禁止新增或直接导入 `github.com/mattn/go-sqlite3`。该驱动依赖 CGO，
  会增加本地与 CI 编译、交叉编译和无 C 工具链环境的复杂度。
- 新增测试应保持在 `CGO_ENABLED=0` 下可编译和运行。需要验证数据库查询、方言或
  事务行为时，优先使用项目已有的非 CGO 测试设施或真实数据库集成环境，不得仅为
  测试便利引入 CGO SQLite 驱动。
- 若依赖树的 `go.sum` 已包含 `go-sqlite3` 的间接校验记录，不代表允许在生产代码或
  测试代码中导入它；不得将其新增为 `go.mod` 直接依赖。
