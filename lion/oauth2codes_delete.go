// Code generated by ent, DO NOT EDIT.

package lion

import (
	"context"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/grpc-kit/pkg/lion/oauth2codes"
	"github.com/grpc-kit/pkg/lion/predicate"
)

// OAuth2CodesDelete is the builder for deleting a OAuth2Codes entity.
type OAuth2CodesDelete struct {
	config
	hooks    []Hook
	mutation *OAuth2CodesMutation
}

// Where appends a list predicates to the OAuth2CodesDelete builder.
func (_d *OAuth2CodesDelete) Where(ps ...predicate.OAuth2Codes) *OAuth2CodesDelete {
	_d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query and returns how many vertices were deleted.
func (_d *OAuth2CodesDelete) Exec(ctx context.Context) (int, error) {
	return withHooks(ctx, _d.sqlExec, _d.mutation, _d.hooks)
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *OAuth2CodesDelete) ExecX(ctx context.Context) int {
	n, err := _d.Exec(ctx)
	if err != nil {
		panic(err)
	}
	return n
}

func (_d *OAuth2CodesDelete) sqlExec(ctx context.Context) (int, error) {
	_spec := sqlgraph.NewDeleteSpec(oauth2codes.Table, sqlgraph.NewFieldSpec(oauth2codes.FieldID, field.TypeInt))
	if ps := _d.mutation.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	affected, err := sqlgraph.DeleteNodes(ctx, _d.driver, _spec)
	if err != nil && sqlgraph.IsConstraintError(err) {
		err = &ConstraintError{msg: err.Error(), wrap: err}
	}
	_d.mutation.done = true
	return affected, err
}

// OAuth2CodesDeleteOne is the builder for deleting a single OAuth2Codes entity.
type OAuth2CodesDeleteOne struct {
	_d *OAuth2CodesDelete
}

// Where appends a list predicates to the OAuth2CodesDelete builder.
func (_d *OAuth2CodesDeleteOne) Where(ps ...predicate.OAuth2Codes) *OAuth2CodesDeleteOne {
	_d._d.mutation.Where(ps...)
	return _d
}

// Exec executes the deletion query.
func (_d *OAuth2CodesDeleteOne) Exec(ctx context.Context) error {
	n, err := _d._d.Exec(ctx)
	switch {
	case err != nil:
		return err
	case n == 0:
		return &NotFoundError{oauth2codes.Label}
	default:
		return nil
	}
}

// ExecX is like Exec, but panics if an error occurs.
func (_d *OAuth2CodesDeleteOne) ExecX(ctx context.Context) {
	if err := _d.Exec(ctx); err != nil {
		panic(err)
	}
}
