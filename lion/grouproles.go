...
func (*GroupRoles) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case grouproles.FieldID, grouproles.FieldCreatedBy, grouproles.FieldUpdatedBy, grouproles.FieldGroupID, grouproles.FieldRoleID:
			values[i] = new(sql.NullInt64)
		case grouproles.FieldCreatedAt, grouproles.FieldUpdatedAt:
...
// to the GroupRoles fields.
func (_m *GroupRoles) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case grouproles.FieldID:
...
// Value returns the ent.Value that was dynamically selected and assigned to the GroupRoles.
...
func (_m *GroupRoles) QueryLionGroups() *GroupsQuery {
	return NewGroupRolesClient(_m.config).QueryLionGroups(_m)
}

// QueryLionRoles queries the "lion_roles" edge of the GroupRoles entity.
func (_m *GroupRoles) QueryLionRoles() *RolesQuery {
	return NewGroupRolesClient(_m.config).QueryLionRoles(_m)
}

// Update returns a builder for updating this GroupRoles.
// Note that you need to call GroupRoles.Unwrap() before calling this method if this GroupRoles
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *GroupRoles) Update() *GroupRolesUpdateOne {
	return NewGroupRolesClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the GroupRoles entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *GroupRoles) Unwrap() *GroupRoles {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("lion: GroupRoles is not a transactional entity")
	}
...
func (_m *GroupRoles) String() string {
	...
	builder.WriteString("GroupRoles(")
